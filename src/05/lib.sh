...
monitoring() {
    ...
    top_files=$(find $dir -type f -exec du -h {} \; | sort -rh | awk -F . '{if (NF>1) {print $0" "$NF} else {print $0" - "}}' | awk '{print NR" - "$2", "$1", "$3}' | head -n 10 | column -t)
    echo "$top_files"
    ...
    end_time=$(date "+%s.%2N")
    time=$(echo "$end_time - $start_time" | bc | awk -F . '{if ($1 == "") {$1=0}}; {print $1"."$2}')
